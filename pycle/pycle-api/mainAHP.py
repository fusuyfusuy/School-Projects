...
userTypeWeights = [5, 3, 2] # mountain, hybrid, road
priorities = {
    'priceWeight': 1,
    'priceType': 1,
    'priceFrame': 1,
...
# 1 olanlar kendileri ile kıyas kısımları
# x olanlar verinin girileceği yerler. boşlukları program tamamlıyor
#         | price | weight | type  | frame | tech  |
#---------------------------------------------------
# price   |   1   |    x   |   x   |   x   |   x   |
#---------------------------------------------------
# weight  |       |    1   |   x   |   x   |   x   |
...
def frameFixer(frame):
    if(frame == '------'):
        # raise Exception('frameFixer error ==> ', frame)
        return(frame)
...
def frameCalculate(left, right, userFrame):
    leftMin = frameSearcher(left, userFrame) + 1 # 0
    rightMin = frameSearcher(right, userFrame) + 1 # 2 7/5 3
    leftMin = 7 - leftMin
    rightMin = 7 - rightMin
...
for i in range(len(bicycles)):
    ...
    for j in range(len(bicycles)):
        ...
        leftType = bicycles[i]['Type'] # Hybrid - City, Mountain, Road
        ...
        leftPriceDiff = (abs(leftPrice - userPrice) / 300) + 1 # burda int alinabilir belki
        rightPriceDiff = (abs(rightPrice - userPrice) / 300) + 1

        priceCompare[i].append(rightPriceDiff/leftPriceDiff) # ters cunku tercih edilesi olan dusuk puanli olan

        # WEIGHT RATIO DATA OLDUGUNDAN DISARDA YAPILIYOR
...
# # individual checks
# print(userFrame)
# for i in range(len(frameCompareAvg)):
#     print(bicycles[i]['Model'], '   ===   ',frameCompareAvg[i],'   ===   ', frameFixer(bicycles[i]['Frame Size']))

# print(userType)
# for i in range(len(frameCompareAvg)):
#     print(bicycles[i]['Model'], '   ===   ',typeCompareAvg[i],'   ===   ', bicycles[i]['Type'])

# print(userPrice)
# for i in range(len(frameCompareAvg)):
#     print(bicycles[i]['Model'], '   ===   ',priceCompareAvg[i],'   ===   ', bicycles[i]['Price'])

# print('weight')
# for i in range(len(frameCompareAvg)):
#     print(bicycles[i]['Model'], '   ===   ',weightCompareAvg[i],'   ===   ', bicycles[i]['Weight '])

# print('tech')
# for i in range(len(frameCompareAvg)):
#     print(bicycles[i]['Model'], '   ===   ', techCompareAvg[i],'   ===   ', brakeCompareAvg[i],' ;; ', transmissionCompareAvg[i],' ;; ', suspensionCompareAvg[i])

# for i in globalPriority:
#     print(i)
