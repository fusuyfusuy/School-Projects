...
function homeAHP(filterArr, priorityArr){
    ...
    let rentRange = filterArr[0];//[300, 600];
    let location = filterArr[1];//['Sydney', 'Double Bay', 'Haymarket', 'Coogee'];
    let petFilter = filterArr[2];//3; // 0 no pet, 1 dog, 2 cat, 3 okay all
    let smokingFilter = filterArr[3];//2; // 0 no smoke, 1 outdoor, 2 indoor
    //// priorities 
    ...
    // let filteredArr = homes.filter(h => h.bedroomInfo.rent > rentRange[0]);
    // filteredArr = filteredArr.filter(h => h.bedroomInfo.rent < rentRange[1]);
    ...
    for (i in filteredArr) {
        ...
        for (j in filteredArr) {
            ...
            leftHome = filteredArr[i].bedroomInfo.rent;
            topHome = filteredArr[j].bedroomInfo.rent;
            let endPoint = 0;
