...
function ownerAHP(filterArr, priortyArr){
    ...
    let overnightFilter = filterArr[0]; // 1 for yes, 0 for no
    let genderFilterSex =  filterArr[1]; // 1 for male, 2 for female
    let genderFilterAppeal = filterArr[2]; // 1 for straight, 2 for bi/gay
    let petFilter =  filterArr[3]; // 0 no pet, 1 dog, 2 cat, 3 okay all
    let smokingFilter =  filterArr[4]; // 0 no smoke, 1 outdoor, 2 indoor/all
    //// priorities 
